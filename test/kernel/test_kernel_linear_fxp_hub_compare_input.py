...
def test(rounding = "round", abs_err = True, et_cycle = None):
    ...
    for bitwidth in range(5, 10):
        bitwidth_tuple = (bitwidth, 9)
        cycle = 2**(bitwidth-1) # standard cycle no et
        # et_cycle = 16 # if no et
        et_cycle = cycle-2 # Override: one cycle before max length
...
abs_err = True
et_cycle = 14
ufc_err_min_list, ufc_err_max_list, ufc_err_mean_list, ufc_err_std_list, cfc_err_min_list, cfc_err_max_list, cfc_err_mean_list, cfc_err_std_list, ifc_err_min_list, ifc_err_max_list, ifc_err_mean_list, ifc_err_std_list, x_label = test(rounding, abs_err, et_cycle)
...
ax.set_yticklabels(["0.00", "0.005", "0.001"])
ax.set_ylim(0, 0.01)
...
fig.savefig(path + f"test_kernel_linear_fxp_hub_tlut_compare_abs_err_et{et_cycle}_input.pdf", bbox_inches='tight', dpi=my_dpi, pad_inches=0.02)
...
plt.show()
path = "/home/zhewen/Repo/UnarySim/test/kernel/"
fig.savefig(path + f"test_kernel_linear_fxp_hub_tlut_compare_rel_err_et{et_cycle}_input.pdf", bbox_inches='tight', dpi=my_dpi, pad_inches=0.02)
